...
def change_matrix(reaction_model, species): 
    reactions = np.zeros((len(reaction_model.keys()),len(species)))
    
    reaction_type = list(reaction_model.values())
...
def Simulate_Division(species, reactions, tmax, sampling_time, cell=1, deterministic=False, size=2.0, division_time=18):
    # Size params setup
